...
export default function HomeScreen({navigation}: {navigation: any}) {
  ...
  const measureStep = useCallback(
    (stepIndex: number) => {
      ...
      const doMeasure = () => {
        ref?.current?.measureInWindow(
          (x: number, y: number, w: number, h: number) => {
            if (w > 0 && h > 0) {
              setSpotlightRect({x, y, width: w, height: h, borderRadius: 18});
            }
          },
        );
      };
      ...
      if (scrollY !== null && mainScrollRef.current) {
        ...
        setTimeout(doMeasure, 160);
      }
    },
    [addressRef, searchAreaRef, liveCardRef, quickServicesRef, dealSectionRef],
  );
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.searchArea} ref={searchAreaRef}>
        <SmartSearchBar
      ...
      {searchMode && !walkthroughVisible ? (
        ...
        <ScrollView
          ...
          <View style={s.liveCard} ref={liveCardRef}>
          ...
          {/* What do you need? */}
          <View style={s.card} ref={quickServicesRef}>
          ...
          <View style={s.card} ref={dealSectionRef}>
            <View style={s.cardTitleRow}>
